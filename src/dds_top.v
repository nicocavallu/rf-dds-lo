...
module dds_top(
    ...
    phase_accumulator phase_acc (.clk(clk), .arst_n(arst_n), .fcw_in(fcw_out), .phase_out(phase_out));
    lfsr_rng lfsr (.clk(clk), .arst_n(arst_n), .q(q));
    truncate trunc (.phase_out(phase_out), .q(q), .phase_truncated(phase_truncated));
    sine_lut lut (.clk(clk), .enable(enable), .phase_truncated(phase_truncated), .sine_out(sine_out));
